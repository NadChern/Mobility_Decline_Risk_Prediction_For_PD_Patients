# Dataset A checker-v2 pilot recheck

Checker: `interpretation-contract-v2-pilot`; parser: `explicit-group-claims-v2-pilot`.

Reviewed 30 saved Gemini interpretations. Before: 20 flagged reports with 42 flags. After: 30 pass, 0 need review.

No API calls. Original prompts, responses, interpretation text, and generation flags are unchanged in the source checkpoint. The original records are also copied to `source_records.jsonl`; `records_rechecked.jsonl` contains derived checker statuses.
